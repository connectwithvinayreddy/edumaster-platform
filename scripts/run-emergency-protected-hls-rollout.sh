...
STAGING_BROWSER_STAGES_VALUE="${STAGING_BROWSER_STAGES:-25,50,100}"
PROOF_SCOPE_VALUE="${PROOF_SCOPE:-staging-emergency}"
DEPLOY_PRODUCTION_VALUE="${DEPLOY_PRODUCTION:-1}"
...
if [[ "${DEPLOY_PRODUCTION_VALUE}" == "1" ]]; then
  ...
  (
    ...
    ENV_FILE="${PRODUCTION_ENV_FILE_PATH}" ./safe-production-deploy.sh
